# views.py 顶部，确保导入了所有需要的模型
from django.shortcuts import render, redirect, get_object_or_404  # 注意：添加了 get_object_or_404
from django.http import HttpResponse, JsonResponse
from django.contrib.auth import login, authenticate, logout
from django.contrib.auth.models import User
from django.contrib import messages
from django.views import View
from django.utils.decorators import method_decorator
from django.views.decorators.csrf import csrf_exempt
from django.contrib.auth.decorators import login_required
from django.utils import timezone
from django.views.decorators.http import require_http_methods
import json
from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger
from django.db.models import Q
from .utils import grant_daily_login_points, add_points_transaction

# 导入所有需要的模型
from .models import (
    UserProfile, VerificationLog, EmailVerificationCode, DataAccessLog,
    Announcement, Guide, Feedback, VolunteerActivity, ActivityApplication,
    SystemSetting, SystemSettingManager
)

# 导入表单
from .forms import (
    ActivityForm, ActivityApplicationForm, FeedbackForm,
    VolunteerVerificationForm, OrganizerVerificationForm
)

# 导入工具函数
from .utils import send_verification_email, verify_email_code
# 导入您的工具函数和模型
try:
    from .utils import send_verification_email, verify_email_code
    from .models import UserProfile, VerificationLog
    from .forms import VolunteerVerificationForm, OrganizerVerificationForm
except ImportError:
    # 如果导入失败，先定义空函数占位
    def send_verification_email(email, purpose='register'):
        print(f"[DEBUG] 发送验证码到 {email}, 目的: {purpose}")
        return True, "验证码已发送（调试模式）"
    
    def verify_email_code(email, code, purpose):
        print(f"[DEBUG] 验证 {email} 的验证码: {code}, 目的: {purpose}")
        return True, "验证成功（调试模式）"


# 首页视图
def index(request):
    """首页重定向到门户页面"""
    return redirect('portal')

def portal_view(request):
    """门户导航页面 - 无需登录即可访问"""
    # 导入设置管理器
    from .models import SystemSettingManager
    
    # 获取最新的活动
    show_activities_count = SystemSettingManager.get_setting('show_latest_activities_count', 5)
    latest_activities = VolunteerActivity.objects.filter(
        status='published',
        is_approved=True
    ).order_by('-published_at')[:show_activities_count]
    
    # 获取最新的公告
    show_announcements_count = SystemSettingManager.get_setting('show_latest_announcements_count', 3)
    latest_announcements = Announcement.objects.filter(
        is_published=True
    ).order_by('-created_at')[:show_announcements_count]
    
    # 获取操作指南数据
    guides = Guide.objects.filter(
        is_published=True
    ).order_by('order', '-created_at')[:5]
    
    # 获取系统设置值
    context = {
        'latest_activities': latest_activities,
        'latest_announcements': latest_announcements,
        'guides': guides,
        
        # 系统设置
        'site_name': SystemSettingManager.get_setting('site_name', '桑榆共暖'),
        'site_description': SystemSettingManager.get_setting('site_description', '志愿服务平台'),
        'contact_email': SystemSettingManager.get_setting('contact_email', '2994192894@qq.com'),
        'contact_phone': SystemSettingManager.get_setting('contact_phone', ''),
        'site_address': SystemSettingManager.get_setting('site_address', '北京市海淀区'),
    }
    
    return render(request, 'volunteers/portal.html', context)

class RegisterView(View):
    """注册视图 - 使用系统设置控制功能"""
    
    def get(self, request):
        # 检查是否允许注册
        if not SystemSettingManager.get_setting('enable_registration', True):
            messages.error(request, "当前暂不接受新用户注册")
            return redirect('index')
        
        return render(request, 'volunteers/register.html')
    
    def post(self, request):
        # 检查是否允许注册
        if not SystemSettingManager.get_setting('enable_registration', True):
            messages.error(request, "当前暂不接受新用户注册")
            return redirect('index')
        
        # 检查是否要求邮箱验证
        require_verification = SystemSettingManager.get_setting('enable_email_verification', True)
        
        email = request.POST.get('email')
        code = request.POST.get('verification_code')
        password = request.POST.get('password')
        confirm_password = request.POST.get('confirm_password')
        
        # 检查密码长度要求
        min_password_length = SystemSettingManager.get_setting('password_min_length', 8)
        if len(password) < min_password_length:
            messages.error(request, f"密码长度至少需要 {min_password_length} 位")
            return render(request, 'volunteers/register.html')
        
        # 检查密码复杂度要求
        if SystemSettingManager.get_setting('require_password_complexity', False):
            # 检查密码是否包含大小写字母、数字和特殊字符
            import re
            if not (re.search(r'[A-Z]', password) and 
                    re.search(r'[a-z]', password) and 
                    re.search(r'\d', password) and 
                    re.search(r'[!@#$%^&*(),.?":{}|<>]', password)):
                messages.error(request, "密码必须包含大小写字母、数字和特殊字符")
                return render(request, 'volunteers/register.html')
        
        # 如果要求邮箱验证，验证验证码
        if require_verification:
            success, message = verify_email_code(email, code, 'register')
            if not success:
                messages.error(request, message)
                return render(request, 'volunteers/register.html')
        
        # 创建用户
        try:
            # 使用邮箱作为用户名
            username = email.split('@')[0] + str(User.objects.count())
            user = User.objects.create_user(
                username=username,
                email=email,
                password=password,
                is_active=True
            )
            
            # 创建基础用户资料
            UserProfile.objects.create(
                user=user,
                role='unverified',
                verification_status='unverified'
            )
            
            # 自动登录
            login(request, user)
            
            messages.success(request, "注册成功！请选择您的身份类型进行认证。")
            return redirect('role_selection')
            
        except Exception as e:
            messages.error(request, f"注册失败：{str(e)}")
            return render(request, 'volunteers/register.html')

class SendVerificationCodeView(View):
    """发送验证码视图"""
    
    @method_decorator(csrf_exempt)
    def dispatch(self, *args, **kwargs):
        return super().dispatch(*args, **kwargs)
    
    def post(self, request):
        try:
            # 尝试从JSON获取数据
            try:
                data = json.loads(request.body)
                email = data.get('email')
                purpose = data.get('purpose', 'register')
            except:
                # 如果JSON解析失败，则从POST表单获取
                email = request.POST.get('email')
                purpose = request.POST.get('purpose', 'register')
            
            print(f"[DEBUG] 接收到发送验证码请求: email={email}, purpose={purpose}")
            
            if not email:
                return JsonResponse({'success': False, 'message': '请输入邮箱地址'})
            
            # 调用发送验证码的函数
            success, message = send_verification_email(email, purpose)
            
            return JsonResponse({'success': success, 'message': message})
            
        except Exception as e:
            print(f"[ERROR] 发送验证码时发生异常: {str(e)}")
            return JsonResponse({'success': False, 'message': '服务器内部错误'}, status=500)

class LoginWithCodeView(View):
    """使用验证码登录"""
    
    def get(self, request):
        return render(request, 'volunteers/login_with_code.html')
    
    def post(self, request):
        email = request.POST.get('email')
        code = request.POST.get('verification_code')
        
        # 验证邮箱验证码
        success, message = verify_email_code(email, code, 'login')
        if not success:
            messages.error(request, message)
            return render(request, 'volunteers/login_with_code.html')
        
        # 查找用户
        try:
            user = User.objects.get(email=email)
            if not user.is_active:
                messages.error(request, "账户已被禁用")
                return render(request, 'volunteers/login_with_code.html')
            
            # 登录用户
            login(request, user)
            grant_daily_login_points(user)
            
            # 检查是否需要二次认证
            if hasattr(user, 'profile'):
                if user.profile.verification_status == 'approved':
                    messages.success(request, "登录成功！")
                    return redirect('dashboard')
                else:
                    messages.info(request, "登录成功！请完成身份认证。")
                    return redirect('role_selection')
            else:
                messages.info(request, "登录成功！请完善个人信息。")
                return redirect('role_selection')
                
        except User.DoesNotExist:
            messages.error(request, "该邮箱尚未注册")
            return render(request, 'volunteers/login_with_code.html')

@method_decorator(login_required, name='dispatch')
class RoleSelectionView(View):
    """角色选择视图"""
    
    def get(self, request):
        # 如果已经通过认证，跳转到仪表盘
        if hasattr(request.user, 'profile') and request.user.profile.verification_status == 'approved':
            messages.info(request, "您已经通过认证")
            return redirect('dashboard')
        
        return render(request, 'volunteers/role_selection.html')
    
    def post(self, request):
        role = request.POST.get('role')
        
        if not role:
            messages.error(request, "请选择一个身份类型")
            return redirect('role_selection')
        
        # 保存用户选择的角色到session
        request.session['selected_role'] = role
        
        # 如果选择跳过，重定向到首页
        if role == 'skip':
            return redirect('dashboard')
        
        # 否则重定向到相应的认证页面
        return redirect('profile_verification')

@login_required
def profile_verification(request):
    """身份认证表单视图 - 使用自动审核设置"""
    # 获取用户选择的角色
    selected_role = request.session.get('selected_role')
    
    if not selected_role or selected_role == 'skip':
        messages.info(request, "请先选择身份类型")
        return redirect('role_selection')
    
    # 获取或创建用户资料
    try:
        profile = UserProfile.objects.get(user=request.user)
        # 更新角色
        profile.role = selected_role
        profile.save()
    except UserProfile.DoesNotExist:
        # 创建新的用户资料
        profile = UserProfile.objects.create(
            user=request.user,
            role=selected_role,
            verification_status='pending'
        )
    
    # 如果已经通过认证，跳转到仪表盘
    if profile.verification_status == 'approved':
        messages.info(request, "您已经通过认证")
        return redirect('dashboard')
    
    # 根据角色选择表单
    if selected_role == 'volunteer':
        form_class = VolunteerVerificationForm
        template = 'volunteers/volunteer_verification.html'
    else:  # organizer
        form_class = OrganizerVerificationForm
        template = 'volunteers/organizer_verification.html'
    
    if request.method == 'POST':
        form = form_class(request.POST, request.FILES, instance=profile)
        if form.is_valid():
            profile = form.save(commit=False)
            profile.role = selected_role
            
            # 根据设置决定是否自动审核
            if selected_role == 'volunteer':
                auto_approve = SystemSettingManager.get_setting('auto_approve_volunteers', False)
            else:
                auto_approve = SystemSettingManager.get_setting('auto_approve_organizers', False)
            
            if auto_approve:
                profile.verification_status = 'approved'
                profile.reviewed_at = timezone.now()
                profile.review_notes = '自动审核通过'
                messages.success(request, "认证已自动通过！")
            else:
                profile.verification_status = 'pending'
                messages.success(request, "认证申请已提交，管理员将在1-3个工作日内审核")
            
            profile.submitted_at = timezone.now()
            profile.data_consent = form.cleaned_data.get('agree_terms', False)
            profile.save()
            
            # 记录审核日志
            VerificationLog.objects.create(
                profile=profile,
                action='submit',
                performed_by=request.user,
                notes='用户提交认证申请',
                ip_address=request.META.get('REMOTE_ADDR')
            )
            
            return redirect('verification_status')
    else:
        # 初始化表单时，如果已有同意标记，设置 agree_terms
        initial = {}
        if profile.data_consent:
            initial['agree_terms'] = True
        form = form_class(instance=profile, initial=initial)
    
    return render(request, template, {
        'form': form,
        'role': selected_role,
        'profile': profile
    })

    if request.method == 'POST':
        form = form_class(request.POST, request.FILES, instance=profile)
        if form.is_valid():
            profile = form.save(commit=False)
            profile.role = selected_role
            
            # 根据设置决定是否自动审核
            if selected_role == 'volunteer':
                auto_approve = SystemSettingManager.get_setting('auto_approve_volunteers', False)
            else:
                auto_approve = SystemSettingManager.get_setting('auto_approve_organizers', False)
            
            if auto_approve:
                profile.verification_status = 'approved'
                profile.reviewed_at = timezone.now()
                profile.review_notes = '自动审核通过'
                messages.success(request, "认证已自动通过！")
            else:
                profile.verification_status = 'pending'
                messages.success(request, "认证申请已提交，管理员将在1-3个工作日内审核")
            
            profile.submitted_at = timezone.now()
            profile.data_consent = form.cleaned_data.get('agree_terms', False)
            profile.save()
            
            # 记录审核日志
            VerificationLog.objects.create(
                profile=profile,
                action='submit',
                performed_by=request.user,
                notes='用户提交认证申请',
                ip_address=request.META.get('REMOTE_ADDR')
            )
            
            return redirect('verification_status')

@login_required
def verification_status(request):
    """查看认证状态"""
    try:
        profile = UserProfile.objects.get(user=request.user)
        logs = VerificationLog.objects.filter(profile=profile).order_by('-performed_at')
        
        return render(request, 'volunteers/verification_status.html', {
            'profile': profile,
            'logs': logs
        })
    except UserProfile.DoesNotExist:
        messages.info(request, "请先完成身份认证")
        return redirect('role_selection')

def activity_list(request):
    """活动列表视图 - 使用系统设置控制分页"""
    # 获取每页显示的活动数量
    per_page = SystemSettingManager.get_setting('activities_per_page', 10)
    
    # 获取所有已发布的活动
    all_activities = VolunteerActivity.objects.filter(
        status='published',
        is_approved=True
    ).order_by('-published_at')
    
    # 分页处理
    paginator = Paginator(all_activities, per_page)
    page_number = request.GET.get('page', 1)
    page_obj = paginator.get_page(page_number)
    
    # 👇 将当前页的活动列表传递给模板（与模板中 {% for activity in activities %} 对应）
    activities = page_obj.object_list
    
    # 检查用户是否可以报名（始终定义该变量，默认为 False）
    user_can_apply = False
    if request.user.is_authenticated and hasattr(request.user, 'profile'):
        user_profile = request.user.profile
        user_can_apply = (
            user_profile.verification_status == 'approved' and 
            user_profile.role == 'volunteer'
        )
    
    return render(request, 'volunteers/activity_list.html', {
        'activities': activities,          # 用于模板循环
        'page_obj': page_obj,             # 分页对象（可用于翻页导航）
        'user_can_apply': user_can_apply,
        'per_page': per_page,
    })

def activity_detail(request, activity_id):
    """活动详情视图"""
    return HttpResponse(f"活动详情页面（活动ID：{activity_id}）")

def apply_activity(request, activity_id):
    """报名活动视图"""
    return HttpResponse(f"报名活动（活动ID：{activity_id}）")

def dashboard(request):
    """用户仪表盘"""
    # 检查用户认证状态
    if hasattr(request.user, 'profile'):
        profile = request.user.profile
        context = {
            'profile': profile,
            'is_authenticated': profile.verification_status == 'approved'
        }
    else:
        context = {'is_authenticated': False}
    
    return render(request, 'volunteers/dashboard.html', context)

def login_view(request):
    """传统的用户名密码登录"""
    if request.method == 'POST':
        username = request.POST.get('username')
        password = request.POST.get('password')
        user = authenticate(request, username=username, password=password)
        
        if user is not None:
            login(request, user)
            grant_daily_login_points(user)
            return redirect('dashboard')
        else:
            messages.error(request, "用户名或密码错误")
    
    return render(request, 'volunteers/login.html')

def logout_view(request):
    """退出登录"""
    logout(request)
    return redirect('home')


# 活动详情视图
def activity_detail(request, activity_id):
    """活动详情"""
    activity = get_object_or_404(VolunteerActivity, id=activity_id)
    
    # 检查用户是否可以报名
    can_apply = False
    already_applied = False
    application = None
    
    if request.user.is_authenticated and hasattr(request.user, 'profile'):
        user_profile = request.user.profile
        can_apply = (
            activity.can_apply() and
            user_profile.verification_status == 'approved' and
            user_profile.role == 'volunteer'
        )
        
        # 检查是否已经报名
        if can_apply:
            already_applied = ActivityApplication.objects.filter(
                activity=activity,
                volunteer=user_profile
            ).exists()
            
            if already_applied:
                application = ActivityApplication.objects.get(
                    activity=activity,
                    volunteer=user_profile
                )
    
    return render(request, 'volunteers/activity_detail.html', {
        'activity': activity,
        'can_apply': can_apply,
        'already_applied': already_applied,
        'application': application,
    })

# 发布活动视图
@login_required
def create_activity(request):
    """发布活动 - 使用系统设置检查限制"""
    # 检查是否允许创建活动
    if not SystemSettingManager.get_setting('enable_activity_creation', True):
        messages.error(request, "当前不允许创建新活动")
        return redirect('dashboard')
    
    # 检查用户是否有权限发布活动
    if not hasattr(request.user, 'profile') or not request.user.profile.can_publish():
        messages.error(request, "您没有权限发布活动")
        return redirect('dashboard')
    
    # 检查活动发布者是否已达到最大活动数限制
    max_activities = SystemSettingManager.get_setting('max_activities_per_organizer', 10)
    current_activities = VolunteerActivity.objects.filter(
        organizer=request.user.profile,
        status__in=['draft', 'published', 'ongoing']
    ).count()
    
    if current_activities >= max_activities:
        messages.error(request, f"您已达到最大活动数限制（{max_activities}个），无法创建新活动")
        return redirect('dashboard')
    
    # 获取默认的活动最大参与人数
    default_max_participants = SystemSettingManager.get_setting('max_participants_per_activity', 50)
    
    if request.method == 'POST':
        form = ActivityForm(request.POST, user=request.user)
        if form.is_valid():
            activity = form.save(commit=False)
            activity.organizer = request.user.profile
            activity.status = 'draft'
            
            # 如果启用了自动审核
            if SystemSettingManager.get_setting('auto_approve_activities', False):
                activity.status = 'published'
                activity.is_approved = True
                activity.approved_by = request.user
                activity.approved_at = timezone.now()
                messages.success(request, "活动已自动审核通过并发布")
            else:
                activity.status = 'draft'
                messages.success(request, "活动已创建，等待管理员审核")
            
            activity.save()
            return redirect('activity_detail', activity_id=activity.id)
    else:
        # 设置表单默认值
        initial_data = {
            'max_participants': default_max_participants,
        }
        form = ActivityForm(initial=initial_data, user=request.user)
    
    return render(request, 'volunteers/create_activity.html', {
        'form': form,
        'max_activities': max_activities,
        'current_activities': current_activities,
    })

# 报名活动视图
@login_required
def apply_activity(request, activity_id):
    """报名活动"""
    activity = get_object_or_404(VolunteerActivity, id=activity_id)
    
    # 检查用户是否有权限报名
    if not hasattr(request.user, 'profile') or not request.user.profile.can_participate():
        messages.error(request, "您需要先通过志愿者认证才能报名活动")
        return redirect('activity_detail', activity_id=activity_id)
    
    # 检查活动是否可以报名
    if not activity.can_apply():
        messages.error(request, "该活动当前不能报名")
        return redirect('activity_detail', activity_id=activity_id)
    
    # 检查是否已经报名
    if ActivityApplication.objects.filter(activity=activity, volunteer=request.user.profile).exists():
        messages.warning(request, "您已经报名了该活动")
        return redirect('activity_detail', activity_id=activity_id)
    
    # 检查活动是否已满
    if activity.is_full():
        messages.error(request, "该活动报名人数已满")
        return redirect('activity_detail', activity_id=activity_id)
    
    if request.method == 'POST':
        form = ActivityApplicationForm(request.POST)
        if form.is_valid():
            application = form.save(commit=False)
            application.activity = activity
            application.volunteer = request.user.profile
            application.status = 'pending'
            application.save()
            
            # 更新活动报名人数
            activity.current_participants += 1
            activity.save()

            # 报名成功，增加志愿者积分（假设固定 10 分）
            from .utils import add_points_transaction
            add_points_transaction(
                user=request.user,
                transaction_type='signup_activity',
                amount=10,  # 固定积分，可改为从系统设置读取
                source_obj=activity,
                operator=None,
                remark=f'报名活动：{activity.title}'
            )
            
            messages.success(request, "报名成功！请等待组织者审核")
            return redirect('activity_detail', activity_id=activity_id)
    else:
        form = ActivityApplicationForm()
    
    return render(request, 'volunteers/apply_activity.html', {
        'form': form,
        'activity': activity,
    })

    
    return render(request, 'volunteers/portal.html', context)

def search_view(request):
    """搜索功能"""
    query = request.GET.get('q', '').strip()
    results = {
        'activities': [],
        'announcements': [],
        'guides': [],
    }
    
    if query:
        # 搜索活动
        results['activities'] = VolunteerActivity.objects.filter(
            Q(title__icontains=query) | Q(description__icontains=query),
            status='published',
            is_approved=True
        )[:10]
        
        # 搜索公告
        results['announcements'] = Announcement.objects.filter(
            Q(title__icontains=query) | Q(content__icontains=query),
            is_published=True
        )[:10]
        
        # 搜索指南
        results['guides'] = Guide.objects.filter(
            Q(title__icontains=query) | Q(content__icontains=query),
            is_published=True
        )[:10]
    
    return render(request, 'volunteers/search_results.html', {
        'query': query,
        'results': results,
    })

def feedback_view(request):
    """反馈和意见页面"""
    if request.method == 'POST' and request.user.is_authenticated:
        # 已登录用户可提交反馈
        feedback_form = FeedbackForm(request.POST)
        if feedback_form.is_valid():
            content = feedback_form.cleaned_data['content']
            contact = feedback_form.cleaned_data['contact']
            
            # 创建反馈记录
            try:
                from .models import Feedback
                Feedback.objects.create(
                    user=request.user,
                    content=content,
                    contact=contact,
                    status='pending'
                )
                messages.success(request, "感谢您的反馈！")
                return redirect('portal')
            except Exception as e:
                messages.error(request, f"提交反馈失败：{str(e)}")
    else:
        feedback_form = FeedbackForm()
    
    return render(request, 'volunteers/feedback.html', {
        'form': feedback_form,
        'can_submit': request.user.is_authenticated
    })

@login_required
def organizer_activities(request):
    """组织者活动管理页面：列出该组织者发布的所有活动"""
    # 权限检查：只有已认证的组织者可以访问
    if not hasattr(request.user, 'profile') or not request.user.profile.can_publish():
        messages.error(request, "您没有权限访问此页面")
        return redirect('dashboard')

    profile = request.user.profile
    # 获取该组织者的所有活动，按创建时间倒序
    activities = VolunteerActivity.objects.filter(
        organizer=profile
    ).order_by('-created_at')

    # 分页（每页10条）
    paginator = Paginator(activities, 10)
    page_number = request.GET.get('page', 1)
    page_obj = paginator.get_page(page_number)

    context = {
        'page_obj': page_obj,
        'profile': profile,
    }
    return render(request, 'volunteers/organizer_activities.html', context)

@login_required
def edit_activity(request, activity_id):
    """组织者编辑自己的活动"""
    activity = get_object_or_404(VolunteerActivity, id=activity_id)

    # 权限检查：必须是该活动的组织者
    if activity.organizer.user != request.user:
        messages.error(request, "您没有权限编辑此活动")
        return redirect('organizer_activities')

    if request.method == 'POST':
        form = ActivityForm(request.POST, instance=activity)
        if form.is_valid():
            form.save()
            messages.success(request, "活动已更新")
            return redirect('organizer_activities')
    else:
        form = ActivityForm(instance=activity)

    return render(request, 'volunteers/edit_activity.html', {'form': form, 'activity': activity})

@login_required
def delete_activity(request, activity_id):
    """组织者删除自己的活动（草稿状态才允许删除）"""
    activity = get_object_or_404(VolunteerActivity, id=activity_id)

    # 权限检查：必须是该活动的组织者
    if activity.organizer.user != request.user:
        messages.error(request, "您没有权限删除此活动")
        return redirect('organizer_activities')

    # 仅允许删除草稿或已拒绝的活动
    if activity.status not in ['draft', 'rejected']:
        messages.error(request, "只有草稿或已拒绝的活动可以删除")
        return redirect('organizer_activities')

    if request.method == 'POST':
        activity.delete()
        messages.success(request, "活动已删除")
        return redirect('organizer_activities')

    # 如果不是POST请求，显示确认页面（可选）
    return render(request, 'volunteers/confirm_delete_activity.html', {'activity': activity})

# ==================== 积分商城用户端 ====================
from .models import PointsShopItem, UserPoints, PointsOrder

@login_required
def shop_index(request):
    """积分商城首页"""
    items = PointsShopItem.objects.filter(is_active=True).order_by('-created_at')
    # 获取用户积分信息
    try:
        points = UserPoints.objects.get(user=request.user)
    except UserPoints.DoesNotExist:
        points = None
    return render(request, 'volunteers/shop_index.html', {
        'items': items,
        'points': points,
    })

@login_required
def shop_exchange(request, item_id):
    """兑换商品"""
    item = get_object_or_404(PointsShopItem, id=item_id, is_active=True)
    
    # 检查库存
    if item.stock == 0:
        messages.error(request, "商品库存不足")
        return redirect('shop_index')
    
    # 获取用户积分账户
    try:
        points = UserPoints.objects.get(user=request.user)
    except UserPoints.DoesNotExist:
        messages.error(request, "您还没有积分账户")
        return redirect('shop_index')
    
    # 检查积分是否足够
    if points.balance < item.points_required:
        messages.error(request, f"积分不足，还需要 {item.points_required - points.balance} 积分")
        return redirect('shop_index')
    
    if request.method == 'POST':
        from django.db import transaction
        from .utils import add_points_transaction
        
        with transaction.atomic():
            # 扣减用户积分
            add_points_transaction(
                user=request.user,
                transaction_type='exchange',
                amount=-item.points_required,
                source_obj=item,
                operator=None,
                remark=f'兑换商品：{item.name}'
            )
            # 扣减库存（如果库存 > 0）
            if item.stock > 0:
                item.stock -= 1
                item.save()
            # 创建订单
            PointsOrder.objects.create(
                user=request.user,
                item=item,
                quantity=1,
                points_spent=item.points_required,
                status='pending'
            )
        messages.success(request, f"兑换成功！您已兑换 {item.name}")
        return redirect('shop_orders')
    
    # GET 请求显示确认页面
    return render(request, 'volunteers/shop_exchange_confirm.html', {
        'item': item,
        'points': points,
    })

@login_required
def shop_orders(request):
    """用户订单列表"""
    orders = PointsOrder.objects.filter(user=request.user).select_related('item').order_by('-created_at')
    return render(request, 'volunteers/shop_orders.html', {
        'orders': orders,
    })

# ==================== AI 助手 API 视图 ====================
from django.views.decorators.csrf import csrf_exempt
from django.views.decorators.http import require_http_methods
from django.http import JsonResponse
import json
from .models import VolunteerActivity, UserProfile
from .forms import ActivityForm

@login_required
@csrf_exempt
@require_http_methods(["POST"])
def api_create_activity(request):
    """创建活动（AI调用）"""
    user = request.user
    try:
        profile = user.profile
    except UserProfile.DoesNotExist:
        return JsonResponse({'status': 'error', 'message': '请先完善个人资料'}, status=403)

    if not profile.can_publish():
        return JsonResponse({'status': 'error', 'message': '您没有发布活动的权限'}, status=403)

    try:
        data = json.loads(request.body)
    except json.JSONDecodeError:
        return JsonResponse({'status': 'error', 'message': '无效的JSON'}, status=400)

    required_fields = ['title', 'description', 'start_time', 'end_time', 
                       'location', 'address_detail']
    missing = [f for f in required_fields if f not in data]
    if missing:
        return JsonResponse({'status': 'error', 'message': f'缺少字段: {missing}'}, status=400)

    activity_data = {
        'title': data['title'],
        'description': data['description'],
        'activity_type': data.get('activity_type', 'other'),
        'start_time': data['start_time'],
        'end_time': data['end_time'],
        'location': data['location'],
        'address_detail': data['address_detail'],
        'max_participants': data.get('max_participants', 50),
    }
    form = ActivityForm(activity_data, user=user)
    if form.is_valid():
        activity = form.save(commit=False)
        activity.organizer = profile
        activity.status = 'draft'
        activity.save()
        return JsonResponse({
            'status': 'success',
            'id': activity.id,
            'message': '活动已创建，等待管理员审核'
        })
    else:
        return JsonResponse({'status': 'error', 'message': '数据验证失败', 'errors': form.errors}, status=400)


@login_required
@require_http_methods(["GET"])
def api_list_activities(request):
    """获取当前用户发布的活动列表"""
    user = request.user
    try:
        profile = user.profile
    except UserProfile.DoesNotExist:
        return JsonResponse({'status': 'error', 'message': '请先完善个人资料'}, status=403)

    if not profile.can_publish():
        return JsonResponse({'status': 'error', 'message': '您没有权限查看活动列表'}, status=403)

    activities = VolunteerActivity.objects.filter(organizer=profile).order_by('-created_at')
    data = []
    for act in activities:
        data.append({
            'id': act.id,
            'title': act.title,
            'start_time': act.start_time.isoformat(),
            'status': act.get_status_display(),
            'current_participants': act.current_participants,
            'max_participants': act.max_participants,
        })
    return JsonResponse({'status': 'success', 'activities': data})


@login_required
@csrf_exempt
@require_http_methods(["PUT"])
def api_update_activity(request, activity_id):
    """修改活动（AI调用）"""
    user = request.user
    try:
        profile = user.profile
    except UserProfile.DoesNotExist:
        return JsonResponse({'status': 'error', 'message': '请先完善个人资料'}, status=403)

    try:
        activity = VolunteerActivity.objects.get(id=activity_id)
    except VolunteerActivity.DoesNotExist:
        return JsonResponse({'status': 'error', 'message': '活动不存在'}, status=404)

    if activity.organizer != profile:
        return JsonResponse({'status': 'error', 'message': '您无权修改此活动'}, status=403)

    if activity.status not in ['draft', 'published']:
        return JsonResponse({'status': 'error', 'message': '活动状态不允许修改'}, status=400)

    try:
        data = json.loads(request.body)
    except json.JSONDecodeError:
        return JsonResponse({'status': 'error', 'message': '无效的JSON'}, status=400)

    allowed_fields = ['title', 'description', 'start_time', 'end_time', 
                      'location', 'address_detail', 'max_participants']
    update_data = {k: v for k, v in data.items() if k in allowed_fields}

    if not update_data:
        return JsonResponse({'status': 'error', 'message': '没有需要更新的字段'}, status=400)

    form = ActivityForm(update_data, instance=activity, user=user)
    if form.is_valid():
        form.save()
        return JsonResponse({
            'status': 'success',
            'id': activity.id,
            'message': '活动已更新'
        })
    else:
        return JsonResponse({'status': 'error', 'message': '数据验证失败', 'errors': form.errors}, status=400)

@login_required
@csrf_exempt
@require_http_methods(["POST"])
def api_update_activity_compatible(request):
    """
    兼容 POST 方式的更新活动 API（供 Dify 工具使用）
    接收 JSON: {"id": 123, "title": "新标题", ...}
    """
    import json
    try:
        data = json.loads(request.body)
    except json.JSONDecodeError:
        return JsonResponse({'status': 'error', 'message': '无效的JSON'}, status=400)

    activity_id = data.get('id')
    if not activity_id:
        return JsonResponse({'status': 'error', 'message': '缺少活动ID'}, status=400)

    # 复用原有的更新逻辑，但需要获取活动实例
    try:
        activity = VolunteerActivity.objects.get(id=activity_id)
    except VolunteerActivity.DoesNotExist:
        return JsonResponse({'status': 'error', 'message': '活动不存在'}, status=404)

    # 权限检查
    if activity.organizer.user != request.user:
        return JsonResponse({'status': 'error', 'message': '您无权修改此活动'}, status=403)

    # 只允许修改未发布或未开始的活动
    if activity.status not in ['draft', 'published']:
        return JsonResponse({'status': 'error', 'message': '活动状态不允许修改'}, status=400)

    # 允许更新的字段
    allowed_fields = ['title', 'description', 'start_time', 'end_time', 
                      'location', 'address_detail', 'max_participants']
    update_data = {k: v for k, v in data.items() if k in allowed_fields}

    if not update_data:
        return JsonResponse({'status': 'error', 'message': '没有需要更新的字段'}, status=400)

    # 使用表单验证部分字段
    form = ActivityForm(update_data, instance=activity, user=request.user)
    if form.is_valid():
        form.save()
        return JsonResponse({
            'status': 'success',
            'id': activity.id,
            'message': '活动已更新'
        })
    else:
        return JsonResponse({'status': 'error', 'message': '数据验证失败', 'errors': form.errors}, status=400)
